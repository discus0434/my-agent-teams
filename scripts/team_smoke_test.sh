...
TEAM_ROOT="$TMP_ROOT" "$TMP_ROOT/scripts/team_config.sh" session >/dev/null
TEAM_ROOT="$TMP_ROOT" "$TMP_ROOT/scripts/team_config.sh" agent worker-1 >/dev/null
TEAM_ROOT="$TMP_ROOT" "$TMP_ROOT/scripts/team_config.sh" review-field model >/dev/null

identity="$(
  TEAM_ROOT="$TMP_ROOT" \
  TEAM_CONFIG_FILE="$TMP_ROOT/config/agent-team.yaml" \
...
if TEAM_ROOT="$TMP_ROOT" "$TMP_ROOT/scripts/team_claim.sh" T-BAD worker-1 >/dev/null 2>&1; then
  echo "branch mismatch claim unexpectedly succeeded" >&2
  exit 1
fi

message_id="$(TEAM_ROOT="$TMP_ROOT" TEAM_DISABLE_NUDGE=1 "$TMP_ROOT/scripts/team_send.sh" worker-1 task_assigned T-001)"
[[ -n "$message_id" ]]

pending="$(TEAM_ROOT="$TMP_ROOT" "$TMP_ROOT/scripts/team_inbox.sh" worker-1)"
case "$pending" in
  *"$message_id"*) ;;
  *) echo "message was not visible in inbox" >&2; exit 1 ;;
esac

TEAM_ROOT="$TMP_ROOT" "$TMP_ROOT/scripts/team_inbox.sh" worker-1 --mark "$message_id" >/dev/null
pending_after_mark="$(TEAM_ROOT="$TMP_ROOT" "$TMP_ROOT/scripts/team_inbox.sh" worker-1)"
[[ -z "$pending_after_mark" ]]

TEAM_ROOT="$TMP_ROOT" "$TMP_ROOT/scripts/team_claim.sh" T-001 worker-1 >/dev/null
[[ "$(git -C "$worker_1" branch --show-current)" == "task/worker-1/T-001" ]]
if TEAM_ROOT="$TMP_ROOT" "$TMP_ROOT/scripts/team_claim.sh" T-001 worker-2 >/dev/null 2>&1; then
  echo "second claim unexpectedly succeeded" >&2
  exit 1
fi

printf '%s\n' "worker change" > "$worker_1/integration-smoke.txt"
report_file="$(TEAM_ROOT="$TMP_ROOT" "$TMP_ROOT/scripts/team_report.sh" T-001 worker-1 needs-review)"
[[ -f "$report_file" ]]
if PATH="$TMP_BASE/bin:$PATH" TEAM_ROOT="$TMP_ROOT" TEAM_DISABLE_NUDGE=1 "$TMP_ROOT/scripts/team_review.sh" T-001 worker-1 >/dev/null 2>&1; then
...
review_file="$(PATH="$TMP_BASE/bin:$PATH" TEAM_ROOT="$TMP_ROOT" TEAM_DISABLE_NUDGE=1 "$TMP_ROOT/scripts/team_review.sh" T-001 worker-1)"
[[ -f "$review_file" ]]
case "$review_file" in
...
review_message="$(TEAM_ROOT="$TMP_ROOT" "$TMP_ROOT/scripts/team_inbox.sh" worker-1)"
case "$review_message" in
  *"\"type\":\"review\""*) ;;
  *) echo "review message was not returned to worker" >&2; exit 1 ;;
esac

report_file="$(TEAM_ROOT="$TMP_ROOT" "$TMP_ROOT/scripts/team_report.sh" T-001 worker-1 done)"
status_output="$(TEAM_ROOT="$TMP_ROOT" "$TMP_ROOT/scripts/team_status.sh")"
case "$status_output" in
  *"T-001 owner=worker-1 phase=ready-to-integrate"*) ;;
  *) echo "status did not show ready-to-integrate" >&2; exit 1 ;;
esac

integration_file="$(PATH="$TMP_BASE/bin:$PATH" TEAM_ROOT="$TMP_ROOT" "$TMP_ROOT/scripts/team_integrate.sh" T-001 worker-1)"
[[ -f "$integration_file" ]]
grep -q '^Status: integrated$' "$integration_file"
[[ -f "$TMP_ROOT/integration-smoke.txt" ]]

integrated_status="$(TEAM_ROOT="$TMP_ROOT" "$TMP_ROOT/scripts/team_status.sh")"
